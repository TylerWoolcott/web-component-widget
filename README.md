...
Makes most sense to use **web components** for the widget and **React** for the frontend because:
 ...
 - Web components offer clients a shortcut by deploying easy to use and responsive web and mobile assets directly to their pages with just a few lines of Javascript. If I build and host the widget's binary file on my third-party website, the solution is the easiest for the user to use as they can just copy the embed link and include the script tag and web component on their website. No installation required and the styles are fully encapsulated. 
...
## Different architectures pros / cons
 ...
 - **HTML Templates**, an API that provides `<template>` and `<slot>` tags to easily markup UI for reuse, without rendering them
